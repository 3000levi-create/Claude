      ...
      try {
        ...
        const subagentResults = await mapBatched(
          ...
          async (sd) => {
            ...
            try {
              const subs = await node_fs.promises.readdir(subDir, { withFileTypes: true });
              return subs.filter(
...
async function* boundedLines(file, maxLen, onSkip) {
  const stream = node_fs.createReadStream(file, { encoding: "utf-8" });
  let buf = "";
...
const _test = { handlers, isHeavyWorkRequest };
exports._test = _test;
