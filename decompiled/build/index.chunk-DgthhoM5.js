...
try {
  (function () {
    ...
    if (e) {
      ...
      r._sentryDebugIds[e] = "35181713-7fa8-41da-9e07-e2707024e59e";
      r._sentryDebugIdIdentifier = "sentry-dbid-35181713-7fa8-41da-9e07-e2707024e59e";
    }
  })();
} catch {}
...
const _ = require("./index.chunk-B3Z2xpgG.js");
const f = require("./index.chunk-IUD6Pydn.js");
const h = "cowork";
const u = "code";
const D = {
  [h]: {
...
function b(r, e, o) {
  return {
    ...
    fireAt: r.fireAt !== undefined ? new Date(r.fireAt).toISOString() : undefined,
    enabled: r.enabled,
...
async function R(r, e) {
  ...
  if (e.cron && !d.isValidCron(e.cron)) {
    return {
      error: `invalid cron expression: "${e.cron}"`
    };
  }
  const t = E(e.fireAt);
  if (e.fireAt && t === undefined) {
    return {
  ...
  const l = await n.manager.create({
    scheduledTaskId: i,
    filePath: $,
    cronExpression: e.cron,
    fireAt: t,
    model: n.normalizeModel(s.model),
    userSelectedFolders: (w = s.cowork) == null ? undefined : w.folders,
    userSelectedFiles: (p = s.cowork) == null ? undefined : p.files,
    userSelectedProjectUuids: (v = s.cowork) == null ? undefined : v.projectUuids,
    spaceId: (S = s.cowork) == null ? undefined : S.spaceId,
    cwd: (I = s.code) == null ? undefined : I.cwd,
    useWorktree: (y = s.code) == null ? undefined : y.useWorktree,
    sourceBranch: (A = s.code) == null ? undefined : A.sourceBranch,
    dispatchSubscribed: e.subscribed
  });
  if (l) {
    d.logger.info(`[dispatch-scheduled-tasks] created ${l.id} (runner=${o}, cron=${l.cronExpression ?? "-"}, fireAt=${l.fireAt ?? "-"}, subscribed=${!!e.subscribed})`);
    return {
      task: b(l, o, r)
    };
  } else {
...
async function x(r, e) {
  ...
  if (e.cron !== undefined && !d.isValidCron(e.cron)) {
    return {
      error: `invalid cron expression: "${e.cron}"`
    };
  }
  const i = E(e.fireAt);
  if (e.fireAt !== undefined && i === undefined) {
    return {
      error: `invalid fireAt: "${e.fireAt}"`
    };
  }
  ...
  if (e.cron !== undefined) {
    t.cronExpression = e.cron;
  }
  if (i !== undefined) {
    t.fireAt = i;
  }
  if (e.subscribed !== undefined) {
    t.dispatchSubscribed = e.subscribed;
  }
  if (e.title !== undefined) {
    d.logger.info(`[dispatch-scheduled-tasks] update ${e.taskId}: title rename not supported, ignoring`);
  }
