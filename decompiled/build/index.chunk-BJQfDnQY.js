...
try {
  (function () {
    ...
    if (e) {
      ...
      n._sentryDebugIds[e] = "350de794-bf13-49b7-a75e-11d814c80d1a";
      n._sentryDebugIdIdentifier = "sentry-dbid-350de794-bf13-49b7-a75e-11d814c80d1a";
    }
  })();
} catch {}
...
class p extends Error {
  constructor(e) {
    super(R(e));
    this.name = "BuiltinMcpUserDisconnectedError";
  }
}
class b extends Error {
  constructor(e, i) {
    super(f(e));
    this.rawCause = i;
    this.name = "BuiltinMcpSpawnFailedError";
  }
}
...
function E(n, e, i) {
  return k.run(e, () => M(n, e, i, true)).catch(t => {
...
async function M(n, e, i, o) {
  ...
  const r = t == null ? undefined : t.get(e);
  if (!t || !r) {
    throw new Error(`MCP server "${e}" is no longer configured`);
  }
  if (r.phase === "connected" && r.connection) {
    return r.connection;
  }
  if (r.phase === "spawning") {
    throw new Error(`MCP server "${e}" is still connecting`);
  }
  if (i === "tool-call") {
    if (r.desired === "disabled") {
      throw new p(e);
    }
    if (s.isCrashBudgetExhausted(r.budget)) {
      throw new Error(f(e));
    }
  }
  const a = r.epoch;
  s.sweepOrphanedAuthCaches(t.all().map(d => d.config));
  t.transition(e, {
    type: "spawn-started"
  });
  let c;
  try {
    c = await s.connectBuiltinDirectMcp(r.config, undefined, {
    ...
    s.logger.warn("[builtin-mcp] spawn failed", {
      server: e,
      trigger: i,
      exhausted: s.isCrashBudgetExhausted(u == null ? undefined : u.budget),
      error: d instanceof Error ? d.message : String(d)
    });
    throw (u == null ? undefined : u.desired) === "disabled" ? new p(e) : new b(e, d);
  }
  try {
  ...
  if (l.epoch !== a) {
    if (l.desired === "disabled") {
      throw new p(e);
    }
    if (o) {
      return M(n, e, i, false);
    }
    throw new Error(f(e));
  }
  if (l.phase !== "connected" || l.connection !== c) {
    throw new Error(f(e));
  }
  if (i === "tool-call") {
    s.recordBuiltinMcpIncident({
      server: e,
...
async function S(n, e, i) {
  const o = s.mainView;
  if (!o || o.webContents.isDestroyed()) {
    s.logger.warn("[mcpDirectoryBridge] No main view available");
    return [];
  }
  ...
  return new Promise((r, a) => {
    ...
    const c = setTimeout(() => {
      g.delete(t);
      s.logger.warn("[mcpDirectoryBridge] %s request timed out: %s", e, t);
      r([]);
    }, D);
    g.set(t, {
      resolve: r,
    ...
    if ((l = s.LocalAgentModeSessions.getDispatcher(o.webContents)) != null) {
      l.dispatchOnEvent({
        type: e,
        sessionId: n,
        data: JSON.stringify({
...
async function _(n, e, i) {
  ...
  return new Promise(r => {
    ...
    if ((c = s.LocalAgentModeSessions.getDispatcher(o.webContents)) != null) {
      c.dispatchOnEvent({
        type: e,
        sessionId: n,
        data: JSON.stringify({
...
function B(n, e, i) {
  const o = s.mainView;
  if (!o || o.webContents.isDestroyed()) {
    s.logger.warn("[pluginSearchBridge] No main view for %s", i);
  ...
  return new Promise(r => {
    ...
    const a = setTimeout(() => {
      h.delete(t);
      s.logger.warn("[pluginSearchBridge] %s request timed out: %s", i, t);
      r(m);
    }, A);
    h.set(t, {
      resolve: r,
