...
function abortRejection(signal, cmd) {
  if (signal.reason instanceof Error) {
    ...
    return new DOMException(`spawnAsync(${cmd}) aborted`, "AbortError");
  }
}
async function spawnAsync(cmd, args = [], options = {}) {
  ...
  try {
    ...
    if (((_a = options.signal) == null ? undefined : _a.aborted) && (error === options.signal.reason || error instanceof Error && error.name === "AbortError")) {
      if (untrusted.cmd !== cmd && error !== options.signal.reason) {
        throw new DOMException(`spawnAsync(${cmd}) aborted (via disclaimer)`, "AbortError");
      }
      throw error;
    }
...
const DEFAULT_MAX_BUFFER = 536870912;
function spawnAsyncDirect(cmd, args = [], options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if ((_a = proc.stdout) == null) {
      undefined;
    } else {
      _a.on("data", onData(stdout));
    }
    if ((_b = proc.stderr) == null) {
      undefined;
    } else {
      _b.on("data", onData(stderr));
    }
    const pgid = process.platform !== "win32" && spawnOptions.detached && typeof proc.pid === "number" ? proc.pid : undefined;
    const killGroup = sig => {
      if (pgid !== undefined) {
        try {
    ...
    const onAbort = () => {
      ...
      setTimeout(() => {
        ...
        killGroup("SIGKILL");
        if (!childExited) {
          if ((_a2 = proc.stdout) == null) {
            undefined;
          } else {
            _a2.destroy();
          }
          if ((_b2 = proc.stderr) == null) {
            undefined;
          } else {
            _b2.destroy();
          }
    ...
    proc.on("close", code => {
      ...
      if (!ignoreExitCode && code !== 0) {
        const error = new Error(`${cmd} exited with code ${code}: ${result.stderr || result.stdout}`);
        error.result = result;
        reject(error);
        return;
      }
...
const COMMON_SHELLS = [{
  ...
  path: "/bin/bash",
  hints: [path.resolve(node_os.homedir(), ".bashrc")]
}, {
  path: "/bin/sh"
}];
function getSafeShell() {
  ...
  if ((envShell == null ? undefined : envShell.startsWith("/")) && node_fs.existsSync(envShell)) {
    return envShell;
  }
  for (const shell of COMMON_SHELLS) {
    if (node_fs.existsSync(shell.path) && ((_a = shell.hints) == null ? undefined : _a.some(hint => node_fs.existsSync(hint)))) {
      return shell.path;
    }
  }
  for (const shell of COMMON_SHELLS) {
...
async function extractPathFromShell() {
  ...
  const {
    stdout
  } = await spawnAsync(shell, ["-l", "-i", "-c", `/bin/sh -c 'printf "%s%s\\n" "${PATH_SENTINEL}" "$PATH"'`], {
  ...
  return ((_a = match == null ? undefined : match[1]) == null ? undefined : _a.trim()) || process.env.PATH || "";
}
...
async function extractShellEnvironment() {
  ...
  const {
    stdout
  } = await spawnAsync(shell, ["-l", "-i", "-c", `echo "${ENV_SENTINEL}"; env`], {
    timeout: SHELL_TIMEOUT_MS,
    env: {
      HOME: process.env.HOME,
      PATH: process.env.PATH,
  ...
  const result = parseEnvDump(stdout.slice(sentinelIdx + ENV_SENTINEL.length + 1).trim());
  if (!result.PATH) {
...
process.parentPort.once("message", e => {
  var _a;
  if (e.data.type !== "init" || !((_a = e.ports) == null ? undefined : _a[0])) {
    process.exit(1);
  }
