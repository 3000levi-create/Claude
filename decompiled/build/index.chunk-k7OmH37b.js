...
var g = (r, t, e) => {
        if (t != null) {
            typeof t != "object" && typeof t != "function" && k("Object expected");
            var i, s;
            e && (i = t[b("asyncDispose")]), i === void 0 && (i = t[b("dispose")], e && (s = i)), typeof i != "function" && k("Object not disposable"), s && (i = function() {
                try {
                    s.call(this)
                } catch (n) {
    ...
    R = (r, t, e) => {
        var i = typeof SuppressedError == "function" ? SuppressedError : function(o, u, c, f) {
                return f = Error(c), f.name = "SuppressedError", f.error = o, f.suppressed = u, f
            },
            s = o => t = e ? new i(o, t, "An error was suppressed during disposal") : (e = !0, o),
            n = o => {
                for (; o = r.pop();) try {
                    var u = o[1] && o[1].call(o[2]);
                    if (o[0]) return Promise.resolve(u).then(n, c => (s(c), n()))
                } catch (c) {
                    s(c)
                }
                if (e) throw t
...
try {
    (function() {
        var r = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
            t = new r.Error().stack;
        t && (r._sentryDebugIds = r._sentryDebugIds || {}, r._sentryDebugIds[t] = "71491ac1-8362-466c-86eb-ea6b4606e488", r._sentryDebugIdIdentifier = "sentry-dbid-71491ac1-8362-466c-86eb-ea6b4606e488")
    })()
} catch {}
...
const a = require("./index.chunk-IUD6Pydn.js"),
    m = "project_memory_read",
    p = "project_memory_write",
    O = "MEMORY.md",
    _ = 256 * 1024,
    S = _,
    M = 200,
...
function w(r) {
    if (r.includes("/") || r.includes("\\") || r.includes("\0")) return {
        ok: !1,
        kind: "separator"
    };
    if (h.isAbsolute(r) || h.posix.isAbsolute(r)) return {
        ok: !1,
        kind: "absolute"
    };
    const t = h.normalize(r);
    if (t !== h.basename(t) || t === "." || t === "..") return {
        ok: !1,
        kind: "separator"
    };
    const e = t.replace(/\.[^.]*$/, "").toUpperCase();
    return /^(CON|PRN|AUX|NUL|COM[0-9]|LPT[0-9])$/.test(e) ? {
        ok: !1,
        kind: "reserved"
    } : /\.md$/i.test(t) ? {
        ok: !0,
...
function d(r, t, e, i) {
    l.logCoworkEvent("lam_space_memory_tool_call", {
        tool: r,
        ok: t,
        has_session: e !== void 0,
        ...i
    })
}
async function $(r, t) {
    const e = await T(t);
    if (!e.ok) return d(m, !1, t, {
        error_kind: e.kind
    }), a.textResult(e.error, !0);
    if (r.file !== void 0) {
        const n = w(r.file);
        if (!n.ok) return d(m, !1, t, {
            error_kind: n.kind
        }), a.textResult(`"${r.file}" is not a valid memory filename — pass a bare *.md name with no path separators.`, !0);
        const o = await l.readFileNoFollow(h.join(e.dir, n.leaf));
        return o ? (d(m, !0, t, {
            bytes: o.content.length
        }), a.textResult(JSON.stringify({
            file: n.leaf,
            content: o.content
        }))) : (d(m, !1, t, {
            error_kind: "not_found"
        }), a.textResult(`Memory file "${r.file}" does not exist. Call ${m} with no \`file\` argument to list available files.`, !0))
    }
    let i = [];
    try {
        i = (await v.readdir(e.dir, {
            withFileTypes: !0
        })).filter(o => o.isFile() && /\.md$/i.test(o.name)).map(o => o.name).sort(), i.length > M && (i = i.slice(0, M))
    } catch (n) {
        n.code !== "ENOENT" && l.logger.warn("[space-memory] readdir failed:", n)
    }
    ...
    return d(m, !0, t, {
        ...
        files: i,
        index: (s == null ? void 0 : s.content) ?? null
    }))
}
async function F(r, t) {
    const e = await T(t);
    if (!e.ok) return d(p, !1, t, {
        error_kind: e.kind
    }), a.textResult(e.error, !0);
    const i = w(r.file);
    if (!i.ok) return d(p, !1, t, {
        error_kind: i.kind
    }), a.textResult(`"${r.file}" is not a valid memory filename — pass a bare *.md name with no path separators.`, !0);
    if (Buffer.byteLength(r.content, "utf8") > _) return d(p, !1, t, {
        error_kind: "too_large"
    }), a.textResult(`Content exceeds ${_} bytes. Keep memory notes concise; split large content across topic files.`, !0);
    ...
    try {
        ...
        try {
            ...
            if (y && !y.isFile()) return d(p, !1, t, {
                error_kind: "not_a_file"
            }), a.textResult(`Memory file "${r.file}" exists but is not a regular file.`, !0);
        ...
        return l.logger.error("[space-memory] write failed:", f), d(p, !1, t, {
            error_kind: "io"
        }), a.textResult(`Failed to write memory file "${r.file}".`, !0)
    }
}
...
function A() {
    const r = [{
        ...
        t = new Map(r.map(e => [e.name, e.handler]));
    return {
        tools: r.map(e => ({
            name: e.name,
        ...
        handleCall: async (e, i, s) => {
            const n = t.get(e);
            if (!n) return a.textResult(`Unknown tool: ${e}`, !0);
            try {
                return await n(i, s)
            } catch (o) {
                if (o instanceof l.ZodError) {
                    const u = o.issues.map(c => `${c.path.join(".")||"(root)"}: ${c.message}`).join("; ");
                    return a.textResult(`${e} input invalid: ${u}. Check the tool schema.`, !0)
                }
                return l.logger.error("[space-memory] %s unhandled error:", e, o), a.textResult(`${e} failed.`, !0)
            }
        }
    }
}
