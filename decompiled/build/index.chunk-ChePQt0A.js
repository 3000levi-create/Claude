...
try {
    (function() {
        var r = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
            e = new r.Error().stack;
        e && (r._sentryDebugIds = r._sentryDebugIds || {}, r._sentryDebugIds[e] = "aa9cda2c-47a6-4f26-a350-1f6cea38cc9d", r._sentryDebugIdIdentifier = "sentry-dbid-aa9cda2c-47a6-4f26-a350-1f6cea38cc9d")
    })()
} catch {}
const y = require("node:fs/promises"),
    ...
    q = require("./index.chunk-CSQCh8Uk.js"),
    H = `First, decide whether the user wants to **create a new** scheduled task or **change an existing** one.
...
Finish with a short summary: how many files you touched and what changed.`,
    C = "Reflective pass over your memory files — merge duplicates, fix stale facts, prune the index.";

function G() {
    if (t.isFeatureEnabled("1824824999")) {
        const r = t.getParsedFeatureValueForKey("1004628546", "skillDescription", C, t.stringType()),
            e = t.getParsedFeatureValueForKey("1004628546", "skillPrompt", N, t.stringType());
        return {
...
function J() {
    const r = t.getParsedFeatureValueForKey("3300773012", "skillDescription", 'Create or update a scheduled task that runs automatically. Use when the user says things like "every day", "each morning", "remind me in an hour", "run this at noon", or wants to reschedule an existing task.', t.stringType()),
        e = t.getParsedFeatureValueForKey("3300773012", "skillPrompt", H, t.stringType());
    return {
        name: "schedule",
        description: r,
        prompt: e
...
function X() {
    const r = t.getParsedFeatureValueForKey("4066504968", "skillDescription", "Guided Cowork setup — install a matching plugin, try a skill, connect tools.", t.stringType()),
        e = t.getParsedFeatureValueForKey("4066504968", "skillPrompt", V, t.stringType());
    return {
        name: "setup-cowork",
        description: r,
        prompt: e
...
async function Z(r, e) {
    const i = r.slice(L.length),
        s = await U();
    if (!(s != null && s.skills.some(a => a.name === i))) throw new Error(`Unknown bundled skill id: ${r}`);
    const l = await y.readFile(d.join(j(), `${i}.skill`)),
        n = await t.extractZipBufferToDirectory(l, e);
    return t.logger.debug(`[SkillsPlugin] seeded bundled skill ${i} from asar`), n
}
...
const te = "[SkillsFetcher]",
    ...
    se = 90 * 1e3;
async function W() {
    if (t.getDeploymentMode().usesLocalSkillStorage()) return Q();
    const r = await t.getLastActiveOrg();
    if (!r) return t.logger.warn("Cannot fetch skills: no active organization"), [];
    ...
    try {
        ...
        if (!i.ok) {
            ...
            throw t.logger.error(`Failed to fetch skills list: ${i.status} ${l}`), new Error(`Failed to fetch skills: ${i.status}`)
        }
...
async function B(r, e, i = 3) {
    if (r.startsWith(L)) return Z(r, e);
    const s = await t.getLastActiveOrg();
    if (!s) throw new Error("Cannot fetch skill content: no active organization");
...
class de {
    constructor() {
        this.syncPromise = null, this.manifestMutex = new t.Mutex, this._syncInterval = null, this._resolveFirstSync = null, this._lastPollTime = 0, this._focusHandler = null, this._focusWindow = null, this._failureRetryScheduled = !1, this._syncIntervalMs = K, this.baseDir = d.join($.app.getPath("userData"), le, re), this._firstSyncComplete = new Promise(e => {
    ...
    async getPluginDir() {
        ...
        if (!e) return t.logger.warn("[SkillsPlugin] Cannot get plugin dir: no active organization"), null;
        const i = t.getAccountId();
        return i ? d.join(this.baseDir, e, i) : (t.logger.warn("[SkillsPlugin] Cannot get plugin dir: no account"), null)
    }
    async getPluginPath() {
        ...
        const i = d.join(e, I);
        try {
            if ((await y.readdir(i)).length === 0) return null
        } catch {
            return null
        }
        return e
    ...
    async syncSkills() {
        if (this.syncPromise) return t.logger.info("[SkillsPlugin] Sync already in progress, waiting..."), this.syncPromise;
    ...
    async getLocalSkillFiles(e) {
        ...
        const s = await this.readManifest(i),
            l = s == null ? void 0 : s.skills.find(n => n.name === e);
        if (!l || l.creatorType !== "user") return [];
        try {
            ...
            for (const c of a) {
                ...
                const g = d.join(c.parentPath, c.name).slice(n.length + 1).replace(/\\/g, "/"),
                    o = await y.readFile(d.join(c.parentPath, c.name), "utf-8");
                h.push({
                    path: g,
                    content: o
                })
    ...
    async revealLocalSkill(e) {
        ...
        const s = await this.readManifest(i),
            l = s == null ? void 0 : s.skills.find(n => n.name === e);
        !l || l.creatorType !== "user" || this.isValidSkillDirName(i, e) && $.shell.showItemInFolder(t.devirtualizeMsixPath(this.getSkillDir(i, e)))
    }
    async listLocalSkills() {
        const e = await this.getPluginDir();
        if (!e) return [];
        const i = await this.readManifest(e);
        return ((i == null ? void 0 : i.skills) ?? []).filter(s => s.creatorType === "user")
    }
    async setLocalSkillEnabled(e, i) {
        const s = await this.getPluginDir();
        return s ? this.manifestMutex.runExclusive(async () => {
            ...
            return !n || n.creatorType !== "user" ? {
                ok: !1,
                error: `"${e}" is not a user-created skill`
            } : (n.enabled = i, n.updatedAt = new Date().toISOString(), await this.writeManifest(s, {
    ...
    async deleteLocalSkill(e) {
        const i = await this.getPluginDir();
        return i ? this.manifestMutex.runExclusive(async () => {
            const s = await this.readManifest(i);
            if (!s) return {
                ok: !1,
                error: `"${e}" is not a user-created skill`
            ...
            return !l || l.creatorType !== "user" ? {
                ok: !1,
                error: `"${e}" is not a user-created skill`
            } : (await this.removeSkills(i, [l]), await this.writeManifest(i, {
                lastUpdated: Date.now(),
    ...
    async saveLocalSkill(e, i, s, l) {
        ...
        if (!e.trim() || e === "." || e === ".." || /[. ]$/.test(e)) return {
            ok: !1,
            error: `Invalid skill name: "${e}"`
        };
        const a = this.skillDirKey(e);
        return new Set((await P()).map(c => this.skillDirKey(c.name))).has(a) ? {
            ok: !1,
            error: `"${e}" is a built-in skill name`
        } : this.manifestMutex.runExclusive(async () => {
            const c = await this.readManifest(n);
            if (!c && await O(d.join(n, A))) return {
                ok: !1,
                error: "Could not read the skills manifest, not saving"
            };
            const g = c == null ? void 0 : c.skills.find(w => w.name === e);
            if (g && !l) return {
                ok: !1,
                error: "already_exists"
            };
            if (g && g.creatorType !== "user") return {
                ok: !1,
                error: `"${e}" is not a user-created skill`
            };
            const o = c == null ? void 0 : c.skills.find(w => w.name !== e && this.skillDirKey(w.name) === a);
            if (o) return {
                ok: !1,
            ...
            await t.mkdirPrivate(p), await t.writeFilePrivate(d.join(p, "SKILL.md"), s);
            const S = {
                    skillId: e,
                    name: e,
                    description: i,
                    ...
                    enabled: (g == null ? void 0 : g.enabled) ?? !0
                },
                v = [...(c == null ? void 0 : c.skills.filter(w => w.name !== e)) ?? [], S];
            return await this.writeManifest(n, {
                lastUpdated: Date.now(),
                skills: v
            }), {
                ok: !0
    ...
    async _syncSkills() {
        ...
        const e = await this.getPluginDir();
        if (!e) return t.logger.warn("[SkillsPlugin] Cannot sync: no plugin directory"), {
            downloaded: 0,
            removed: 0
        };
        const i = await this.getBuiltInSkillMetadata(),
            s = new Set(i.map(l => l.name));
        await this.ensurePluginStructure(e), await this._writeBuiltInSkillsTo(e), await this.manifestMutex.runExclusive(async () => {
            const l = d.join(e, A),
                n = await this.readManifest(e);
            if (!n && await O(l)) {
                let h = !1;
                try {
            ...
            const a = i.every(h => ((n == null ? void 0 : n.skills) ?? []).some(c => c.name === h.name && c.creatorType === "anthropic"));
            if (!n || !a) {
                const h = (n == null ? void 0 : n.skills.filter(c => !s.has(c.name))) ?? [];
        ...
        try {
            ...
            if (!n) return t.logger.warn("[SkillsPlugin] Manifest read failed; skipping this sync round"), {
                downloaded: 0,
                removed: 0
            };
            const a = new Set(i.map(u => this.skillDirKey(u.name))),
                h = new Set(n.skills.filter(u => u.syncManaged === !1).map(u => this.skillDirKey(u.name))),
                c = new Set,
                g = l.filter(u => {
                    if (!this.isValidSkillDirName(e, u.name)) return t.logger.warn(`[SkillsPlugin] Remote skill name ${JSON.stringify(u.name)} cannot map to a skill directory, skipping`), !1;
                    const f = this.skillDirKey(u.name);
                    return a.has(f) ? (t.logger.warn(`[SkillsPlugin] Built-in skill "${u.name}" conflicts with remote skill, built-in takes precedence`), !1) : h.has(f) ? (t.logger.warn(`[SkillsPlugin] Local skill "${u.name}" conflicts with remote skill, local skill takes precedence`), !1) : c.has(f) ? (t.logger.warn(`[SkillsPlugin] Remote skill "${u.name}" maps to the same directory as an earlier remote skill, skipping`), !1) : (c.add(f), !0)
                });
            if (g.length === 0) {
                const u = n.skills.filter(f => !a.has(this.skillDirKey(f.name)) && !h.has(this.skillDirKey(f.name)) && f.syncManaged !== !1);
                return await this.removeSkills(e, u), await this.manifestMutex.runExclusive(async () => {
                    ...
                    const _ = f.skills.filter(x => x.syncManaged === !1),
                        D = [...i, ..._];
                    await this.writeManifest(e, {
                        lastUpdated: Date.now(),
                        skills: D
                    }), await this.cleanupOrphanDirs(e, D)
                }), t.logger.info("[SkillsPlugin] Sync complete: no remote skills, built-in skills written"), this._failureRetryScheduled = !1, {
                    downloaded: 0,
                    removed: 0
                }
            }
            const {
                toDownload: o,
                toRemove: p
            } = await this.calculateDelta(e, g, (n == null ? void 0 : n.skills) ?? []);
            t.logger.info(`[SkillsPlugin] Delta: ${o.length} to download, ${p.length} to remove`);
            const S = await this.downloadSkills(e, o),
                v = o.length - S.size;
            p.length > 0 && await this.removeSkills(e, p);
            const w = new Map(((n == null ? void 0 : n.skills) ?? []).filter(u => !s.has(u.name) && u.syncManaged !== !1).map(u => [u.name, u])),
                M = g.flatMap(u => {
                    if (!S.has(u.name)) return [u];
                    const f = w.get(u.name);
                    return f ? [f] : []
                }),
                T = await this.manifestMutex.runExclusive(async () => {
                    const u = await this.readManifest(e);
                    if (!u) return t.logger.warn("[SkillsPlugin] Manifest read failed at write time; skipping manifest rewrite and orphan cleanup this sync"), null;
                    const f = u.skills.filter(D => D.syncManaged === !1),
                        _ = [...M, ...i, ...f];
                    return await this.writeManifest(e, {
                        lastUpdated: Date.now(),
                        skills: _
                    }), this.cleanupOrphanDirs(e, _)
                });
            return S.size > 0 ? this._failureRetryScheduled || (this._failureRetryScheduled = !0, this.triggerSync()) : this._failureRetryScheduled = !1, T === null ? {
                downloaded: v,
                removed: p.length
            ...
            throw t.logger.error("[SkillsPlugin] Sync failed:", l), l
        }
    }
    async calculateDelta(e, i, s) {
        const l = new Map(s.map(o => [o.name, o])),
            n = new Map(i.map(o => [o.name, o])),
            a = [],
            h = [];
        for (const o of i) {
            ...
            await O(d.join(S, "SKILL.md")) || a.push(o)
        }
        const c = new Set((await P()).map(o => this.skillDirKey(o.name))),
            g = new Set(s.filter(o => o.syncManaged === !1).map(o => this.skillDirKey(o.name)));
        for (const o of s) !n.has(o.name) && !c.has(this.skillDirKey(o.name)) && !g.has(this.skillDirKey(o.name)) && o.syncManaged !== !1 && h.push(o);
        return {
            toDownload: a,
            toRemove: h
        }
    ...
    async downloadSkills(e, i) {
        const s = new t.PQueue({
            ...
            l = new Set;
        return await s.addAll(i.map(n => async () => {
            try {
                const a = this.getSkillDir(e, n.name);
                await B(n.skillId, a), t.logger.debug(`[SkillsPlugin] Downloaded skill: ${n.name}`)
            } catch (a) {
                t.logger.error(`[SkillsPlugin] Failed to download ${n.name}:`, a), l.add(n.name)
            }
        })), l
    }
    ...
    async cleanupOrphanDirs(e, i) {
        const s = d.join(e, I),
            l = new Set(i.map(n => this.skillDirKey(n.name)));
    ...
    getSkillDir(e, i) {
        const s = this.sanitizeSkillName(i),
            ...
            n = d.join(l, s),
            a = d.relative(l, n);
        if (a === "" || d.isAbsolute(a) || a.startsWith("..") || /[. ]$/.test(s)) throw new Error(`Invalid skill name: "${i}"`);
        return n
    }
    async ensurePluginStructure(e) {
        ...
        const i = d.join(e, ae),
            s = d.join(i, oe);
        await O(s) || (await t.mkdirPrivate(i), await t.writeJsonAtomic(s, {
    ...
    async readManifest(e) {
        ...
        try {
            ...
            return s.code !== "ENOENT" && t.logger.warn("[SkillsPlugin] Failed to read manifest:", s), null
        }
    }
    ...
    async clearCache() {
        const e = await this.getPluginDir();
        e && await this.clearPluginDir(e)
    }
    triggerSync() {
        t.logger.info("[SkillsPlugin] Triggered sync (5s delay)"), setTimeout(() => {
            this.periodicSync()
        }, 5e3)
    }
    startPeriodicSync() {
        this._syncInterval || (this._firstSyncComplete = new Promise(e => {
            this._resolveFirstSync = e
        }), this._syncIntervalMs = t.getParsedFeatureValueForKey("1978029737", "skillsSyncIntervalMs", K, t.numberType()), t.logger.info(`[SkillsPlugin] Starting periodic sync (interval: ${this._syncIntervalMs}ms)`), this._syncInterval = setInterval(() => {
            if (!t.isMainWindowFocused()) {
                t.logger.debug("[SkillsPlugin] Skipping periodic sync — window not focused");
                return
            }
            this.periodicSync()
        }, this._syncIntervalMs), this._focusHandler = () => {
            const e = Date.now() - this._lastPollTime;
            e >= this._syncIntervalMs && (t.logger.info("[SkillsPlugin] Window focused — polling now (last poll was %dms ago)", e), this.periodicSync())
        }, t.mainWindow && !t.mainWindow.isDestroyed() && (this._focusWindow = t.mainWindow, this._focusWindow.on("focus", this._focusHandler)), this.periodicSync())
    }
    stopPeriodicSync() {
        this._syncInterval && (clearInterval(this._syncInterval), this._syncInterval = null), this._focusHandler && this._focusWindow && !this._focusWindow.isDestroyed() && this._focusWindow.removeListener("focus", this._focusHandler), this._focusHandler = null, this._focusWindow = null, this.syncPromise = null
    }
    async periodicSync() {
        if (this.syncPromise) {
        ...
        try {
            ...
            this.syncPromise === i && (this.syncPromise = null), e && (e(), this._resolveFirstSync === e && (this._resolveFirstSync = null))
        }
    }
...
`);
    ...
    async generateSkillsSystemPrompt(e, i, s, l, n, a, h = !1, c = !0, g = !0, o = !0, p = !1, S = !1) {
        const v = [];
        let w = !1;
        ...
        if (M) {
            ...
            if (m != null && m.skills) {
                const R = new Set((await P()).filter(k => {
                    var F;
                    return ((F = k.isEnabled) == null ? void 0 : F.call(k)) === !1
                }).map(k => k.name));
                for (const k of m.skills) {
                    if (!k.enabled || R.has(k.name)) continue;
                    if (S && k.name === ue) {
                        w = !0;
                        continue
                    }
                    if (!c && k.creatorType !== "anthropic") continue;
        ...
        const [T, u] = await Promise.all([g ? q.remotePluginManager.getPluginSkillsForSystemPrompt(e, l, n) : [], i && s ? t.localPluginsReader.getLocalPluginSkillsForSystemPrompt(e, i, s, n) : []]), f = [...T, ...u], _ = new Set(v.map(m => m.name)), D = [...v, ...f];
        if (t.isFeatureEnabled("2800354941") && D.sort((m, R) => m.name.localeCompare(R.name)), D.length === 0) return {
            prompt: null,
            pluginSkills: f,
            managedSkillNames: _
        };
