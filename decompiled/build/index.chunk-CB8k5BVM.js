...
try {
    (function() {
        var i = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
            d = new i.Error().stack;
        d && (i._sentryDebugIds = i._sentryDebugIds || {}, i._sentryDebugIds[d] = "d78e1732-af91-4537-92f6-f40cedcfdfb4", i._sentryDebugIdIdentifier = "sentry-dbid-d78e1732-af91-4537-92f6-f40cedcfdfb4")
    })()
} catch {}
...
const v = require("./index.chunk-CvbeGVMj.js"),
    ...
    w = 'Tool approvals granted during a run are stored on the task and auto-applied to future runs. If this task is likely to use remote connectors or browser control, recommend the user click "Run now" first to pre-approve the tools it needs — this prevents future runs from pausing on permission prompts.';
async function $(i, d) {
    try {
        ...
        const y = await Promise.all(p.map(async r => {
            ...
            let n;
            r.fireAt ? n = `One-time: ${new Date(r.fireAt).toLocaleString()}` : r.cronExpression ? n = e.cronToHumanReadable(e.applyJitterToCron(r.cronExpression, Math.round(g / 60))) : n = "Manual only";
            let f;
            r.fireAt && !r.lastRunAt && r.enabled ? f = new Date(r.fireAt).toISOString() : r.cronExpression && r.enabled && (f = new Date(e.getNextRunTime(r.cronExpression).getTime() + g * 1e3).toISOString());
            let c = "";
            try {
            ...
            return {
                ...
                fireAt: r.fireAt ? new Date(r.fireAt).toISOString() : void 0,
                enabled: r.enabled,
                nextRunAt: f,
        ...
        return e.logger.error("[ScheduledTasksMcpServer] Failed to list scheduled tasks:", p), {
            content: [{
                type: "text",
...
async function O(i, d, p, y, r, g, n) {
    ...
    try {
        ...
        let m;
        if (a) {
            if (m = new Date(a).getTime(), isNaN(m)) return {
                content: [{
                    type: "text",
        ...
        if (!await d.create({
                ...
                cronExpression: t,
                fireAt: m,
                userSelectedFolders: o == null ? void 0 : o.userSelectedFolders,
                userSelectedFiles: o == null ? void 0 : o.userSelectedFiles,
                userSelectedProjectUuids: o == null ? void 0 : o.userSelectedProjectUuids,
                spaceId: o == null ? void 0 : o.spaceId,
                cwd: o == null ? void 0 : o.cwd,
                notifySessionId: (h ?? !0) && !b ? l : void 0
            })) return {
            content: [{
                type: "text",
        ...
        if (e.logEvent(n.evCreated, {
                scheduled_task_id: u,
                has_cron: !!t,
                has_fire_at: !!m,
                has_notify_on_completion: h ?? !0,
                mcp_tool: !0,
...
${w}`
            ...
            const k = d.getJitterSecondsForTask(u),
                T = e.applyJitterToCron(t, Math.round(k / 60)),
                E = new Date(e.getNextRunTime(t).getTime() + k * 1e3),
                I = e.cronToHumanReadable(T);
            return {
                content: [{
                    type: "text",
                    text: `Scheduled task "${u}" created.
...
${w}`
        ...
        return e.logger.error("[ScheduledTasksMcpServer] Failed to create scheduled task:", l), {
            content: [{
                type: "text",
...
async function D(i, d, p, y, r, g) {
    ...
    try {
        ...
        if (s !== void 0 && !e.isValidCron(s)) return {
            content: [{
                type: "text",
        ...
        let u;
        if (t) {
            if (u = new Date(t).getTime(), isNaN(u)) return {
                content: [{
                    type: "text",
        ...
        const _ = f !== void 0 || c !== void 0;
        if (!_ && !(s !== void 0 || t !== void 0 || a !== void 0 || h !== void 0)) return {
            content: [{
                type: "text",
        ...
        if (a === !0 && !t && !s && m.fireAt && m.lastRunAt) return {
            content: [{
                type: "text",
        ...
        _ && await e.updateScheduledTaskFile(m.filePath, {
            prompt: f,
            description: c
        }, g.taskFilesDir);
        const o = {};
        if (s !== void 0 && (o.cronExpression = s, s && (o.enabled = !0)), u !== void 0 && (o.fireAt = u, o.enabled = !0), a !== void 0 && (o.enabled = a), h !== void 0 && (o.notifySessionId = h ? y() : void 0), !await d.update(n, o)) return {
            content: [{
                type: "text",
        ...
        e.logEvent(g.evUpdated, {
            scheduled_task_id: n,
            updated_prompt: f !== void 0,
            updated_description: c !== void 0,
            updated_cron: s !== void 0,
            updated_fire_at: t !== void 0,
            updated_enabled: a !== void 0,
            updated_notify_on_completion: h !== void 0,
            mcp_tool: !0,
        ...
        const k = [];
        if (f !== void 0 && k.push("prompt"), c !== void 0 && k.push("description"), s !== void 0) {
            const T = d.getJitterSecondsForTask(n),
                E = e.applyJitterToCron(s, Math.round(T / 60));
            k.push(`schedule (${e.cronToHumanReadable(E)})`)
        }
        return u !== void 0 && k.push(`one-time run at ${new Date(u).toLocaleString()} (${e.formatNextRun(new Date(u))})`), a !== void 0 && k.push(a ? "enabled" : "disabled"), h !== void 0 && k.push(h ? "completion notifications enabled" : "completion notifications disabled"), {
            content: [{
                type: "text",
                text: `Scheduled task "${n}" updated: ${k.join(", ")}.${f!==void 0?`

${w}`:""}`
            }]
        }
    } catch (l) {
        return e.logger.error("[ScheduledTasksMcpServer] Failed to update scheduled task:", l), {
            content: [{
                type: "text",
...
async function M(i, d, p, y, r, g, n, f) {
    ...
    try {
        ...
        if (!s) return S(`Scheduled task "${c}" not found. Use list_scheduled_tasks to see available tasks.`);
        const t = y();
        if (t && p(t) === c) return S("Refusing to delete the scheduled task that launched this session. Delete it from a regular session instead.");
        const {
            deleted: a,
            sessionsArchived: h
        ...
        if (!a) return S(`Failed to delete task "${c}": task not found during delete.`);
        ...
        return e.logger.error("[ScheduledTasksMcpServer] Failed to delete scheduled task:", s), S(`Failed to delete scheduled task: ${s instanceof Error?s.message:String(s)}`)
    }
}
