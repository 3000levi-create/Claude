...
try {
    (function() {
        var e = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
            t = new e.Error().stack;
        t && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[t] = "f7311df5-8d70-42c1-b8f4-61b2edbce2bf", e._sentryDebugIdIdentifier = "sentry-dbid-f7311df5-8d70-42c1-b8f4-61b2edbce2bf")
    })()
} catch {}
...
const m = require("node:path"),
    A = require("node:fs/promises"),
    O = require("node:os");
require("node:url");
require("node:path/posix");
require("node:path/win32");
var C = (e => (e.Connected = "connected", e.Working = "working", e.Editing = "editing", e.Disconnected = "disconnected", e))(C || {}),
    p = (e => (e.Excel = "excel", e.PowerPoint = "powerpoint", e.Word = "word", e))(p || {});
...
function Y() {
    ...
    return !e.system_prompt || !e.user_prompt_template ? null : {
...
async function Z(e, t) {
    try {
        ...
        try {
            ...
            return r.logger.warn("[compactionService] Failed to parse messages JSON", {
                error: c instanceof Error ? c.message : String(c)
            }), null
        }
        if (!n || n.length < 2) return null;
        const o = r.DESKTOP_OAUTH_CONFIGS[r.getOAuthEnvironment()],
            i = await r.getApiToken(o);
        if (!i) return r.logger.warn("[compactionService] Cannot compact - no API token available"), null;
        const s = Y();
        if (!s) return null;
        const a = ee(n),
            P = Q(s),
...
${a}`
            ...
            if (!d || d.type !== "tool_use") return r.logger.warn("[compactionService] No tool_use block in response"), null;
            const f = d.input;
            if (!f.summary) return r.logger.warn("[compactionService] No summary in tool input"), null;
            k++;
            const M = Date.now(),
                g = 150,
                B = n.slice(-10).map(b => ({
            ...
            if (clearTimeout(E), c instanceof Error && c.name === "AbortError") r.logger.warn("[compactionService] Haiku API call timed out", {
                timeout: U
            });
            else {
                const d = {
                    message: c instanceof Error ? c.message : String(c),
                    name: c instanceof Error ? c.name : void 0
                };
                if (c && typeof c == "object") {
                    const f = c;
                    f.status && (d.status = f.status), f.error && (d.error = f.error), f.headers && (d.headers = f.headers)
                }
        ...
        return r.logger.error("[compactionService] Compaction failed", {
...
function ee(e) {
    return e.map((n, o) => {
        ...
        let s = n.content;
        return s.length > 1e3 && (s = s.slice(0, 1e3) + "..."), `[${o+1}] ${i}: ${s}`
    }).join(`
...
const ie = "29673e3c-d826-4f00-92ee-162334a52b1a",
    $ = "https://pivot.claude.ai/manifest.xml";
...
function se(e) {
    if (process.platform === "win32") return m.join(process.env.LOCALAPPDATA || m.join(O.homedir(), "AppData", "Local"), "Microsoft", "Office", "16.0", "WEF");
...
function ae(e) {
    ...
    try {
        if (((o = t.readRegistryValues([{
                ...
                keyPath: n,
                valueName: "Url"
            }])[0]) == null ? void 0 : o.value) === e) return;
        t.writeRegistryValue("HKCU", n, "Id", `{${L}}`), t.writeRegistryValue("HKCU", n, "Url", e), t.writeRegistryDword("HKCU", n, "Flags", 1), r.logger.info(`[OfficeFileOperations] Registered WEF folder as trusted catalog: ${e}`)
    } catch (i) {
    ...
    if (process.platform !== "win32") {
        if (process.platform === "darwin") {
            const n = e === "Microsoft Excel" ? "com.microsoft.Excel" : e === "Microsoft Word" ? "com.microsoft.Word" : "com.microsoft.Powerpoint",
                o = m.join(O.homedir(), "Library/Containers", n);
...
async function le(e) {
    ...
    try {
        ...
        s = await a.text(), await r.mkdirPrivate(m.dirname(i)), await r.writeFilePrivate(i, s)
    } catch (a) {
    ...
    await A.mkdir(t, {
        recursive: !0
    }), await ce(e), await A.writeFile(o, s, "utf-8")
}
async function fe(e) {
    const t = m.extname(e).toLowerCase();
    if (!X(t) || !r.getAppPreference("louderPenguinEnabled")) return;
    const n = re(t),
        o = !1;
    try {
        if (o || await le(n), process.platform === "darwin") {
            const i = async () => {
                await pe(n, "Open Claude", "Claude", s)
            }, s = await ue(n), a = s ? 500 : 1e3, P = s ? 1e3 : 3e3;
            let h;
            const y = setTimeout(() => {
                i().catch(() => {
                    h = setTimeout(() => {
            ...
            D.app.once("before-quit", () => {
                clearTimeout(y), h && clearTimeout(h)
            })
        }
    } catch {}
}
  ...
  `;
    return new Promise((E, c) => {
        ...
        d.stdout.on("data", g => {
            ...
            g !== 0 ? c(new Error(`AppleScript failed: ${f.trim()}`)) : E()
        }), d.on("error", g => {
            c(g)
        }), setTimeout(() => {
            d.killed || (d.kill(), c(new Error("AppleScript timed out")))
        }, 1e4)
    })
}
...
async function we(e, t) {
    ...
    if (!(n != null && n.focusOfficeDocument)) return r.logger.warn("[office-addin-ipc] focusOfficeDocument not available in claude-native"), !1;
    let o;
    switch (e) {
        case p.Excel:
            o = "excel";
    ...
    try {
        ...
        return r.logger.error("[office-addin-ipc] Native COM focus failed", {
            app: me(e),
            document: t,
...
async function be(e) {
    ...
    try {
        ...
        if (n) {
            j = Date.now();
            const o = ve();
            o.length > 0 && (n.toolInvocations = o), ke(n)
        }
    } catch (n) {
...
function Ee(e) {
    ...
    if (e.type === "connected" && e.addinId) {
        const o = e.addinId,
            i = u.get(o);
        if (i) i.status = C.Connected, i.document = e.document || i.document, i.deviceId = e.deviceId, i.platform = e.platform, i.lastActivityAt = t, l && l.dispatchOnFileStateChanged(i);
        else {
            const s = !w.has(n),
                a = {
                    id: o,
            ...
            u.set(o, a), s && w.set(n, o), l && l.dispatchOnFileAdded(a)
        }
        _()
        ...
        const o = e.addinId,
            i = u.get(o);
        if (i) {
            if (i.status = C.Disconnected, i.lastActivityAt = t, w.get(i.app) === o) {
                const s = Array.from(u.values()).find(a => a.app === i.app && a.id !== o && a.status !== C.Disconnected);
                s ? w.set(i.app, s.id) : w.delete(i.app)
            }
            l && l.dispatchOnFileStateChanged(i)
        }
        _()
    } else if (e.type === "status_change" && e.addinId && e.status) {
        const o = e.addinId,
            i = u.get(o);
        i && (i.status = e.status, i.lastActivityAt = t, l && l.dispatchOnFileStateChanged(i), _())
    } else if (e.type === "context_update" && e.addinId) {
        const o = e.addinId,
            i = u.get(o);
        i && (i.lastActivityAt = t)
    } else if (e.type === "selection_change" && e.selectedAddinId) {
        const o = H(e.app);
        w.set(o, e.selectedAddinId), F();
        for (const i of u.values()) i.app === o && l && l.dispatchOnFileStateChanged(i);
        _()
    }
}
...
function Ae() {
    return {
        ...
        async focusFile(e) {
            const t = u.get(e);
            if (!t) return r.logger.warn("[office-addin-ipc] Cannot focus file: not found", {
                fileId: e
            }), !1;
            if (t.platform === "OfficeOnline") return t.browser, !1;
            if (await ge(t.app, t.document)) {
                const i = t.app;
                return Oe(i, t.addinId), !0
            }
            if (t.documentPath) try {
                return await D.shell.openPath(t.documentPath), !0
            } catch (o) {
        ...
        async selectFile(e) {
            const t = u.get(e);
            if (!t) return r.logger.warn("[office-addin-ipc] Cannot select file: not found", {
                fileId: e
            }), !1;
            const n = t.app,
                o = await Pe(n, t.addinId);
            if (o) {
                w.set(t.app, e), F();
                for (const i of u.values()) i.app === t.app && l && l.dispatchOnFileStateChanged(i);
                _()
            }
            return o
        },
        ...
        updateActiveConversationSummary(e) {
            e && (e.conversationId !== R && (R = e.conversationId || null, te()), be(e)), e && I() && Ne(e)
        }
    }
}
...
function I() {
    return r.getAppPreference("louderPenguinEnabled") === !0 && r.getAppPreference("quietPenguinEnabled") !== !0
}
...
async function Pe(e, t) {
    const n = S.get(e);
    if (!n) return r.logger.warn("[office-addin-bridge] Cannot select add-in: not connected", {
        app: e,
        addinId: t
    }), !1;
    const o = x.get(e) || [],
        i = o.find(s => s.addinId === t);
    return i ? (r.logger.info("[office-addin-bridge] Selecting add-in", {
        app: e,
        addinId: t,
        document: i.document
    }), V.set(e, {
        ...
        document: i.document,
        deviceId: i.deviceId
    })), Te(e, t), !0) : (r.logger.warn("[office-addin-bridge] Cannot select add-in: not found", {
        app: e,
        addinId: t,
...
function Oe(e, t) {
    ...
    const o = t ? (s = x.get(e)) == null ? void 0 : s.find(a => a.addinId === t) : V.get(e),
        i = {
            type: "focus_addin_input",
            ...(o == null ? void 0 : o.addinId) && {
                target_addin: o.addinId
            },
            ...(o == null ? void 0 : o.document) && {
                target_document: o.document
            },
            ...(o == null ? void 0 : o.deviceId) && {
...
function Me() {
    for (const e of S.values())
        if (e.isPaired && e.isReady) return !0;
    return !1
}
