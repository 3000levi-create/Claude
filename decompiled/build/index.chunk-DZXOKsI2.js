...
try {
  (function () {
    ...
    if (t) {
      ...
      e._sentryDebugIds[t] = "bc057be6-d435-4ee3-be93-b0a3b28edb6b";
      e._sentryDebugIdIdentifier = "sentry-dbid-bc057be6-d435-4ee3-be93-b0a3b28edb6b";
    }
  })();
} catch {}
...
b(f);
const C = e => {
  const t = `**Tier packing:** Compute total width BEFORE placing. Example — 4 pub/sub consumer boxes:
...
- RIGHT: x=50,200,350,500 w=130 gap=20 → fits (4×130 + 3×20 = 580 ≤ 590 safe width; right edge at 630 ≤ 640)`;
  const n = `**Structural container example** (library branch with two side-by-side regions, an internal labeled arrow, and an external input). ViewBox 700x320, horizontal layout, color classes handle both light and dark mode — no \`<style>\` block:
\`\`\`svg
<defs>
...
\`\`\`` : `**Illustrative diagrams at ${e}px**: the full interactive worked example (water-heater cross-section with thermostat slider, animated convection, and right-margin callouts) is omitted here — it was authored at 680px and would need a complete re-layout, not a coordinate scale, to fit. Follow the composition rules above with \`viewBox="0 0 ${e} H"\` directly: lay the subject tall-and-narrow at e.g. x=40..220, reserve x=230..${e - 20} for right-side leader-line labels (\`class="ts"\`, \`text-anchor="start"\`), and stack controls below the SVG instead of beside it.`;
  const o = e > 400 ? `**Illustrative example — abstract subject** (attention in a transformer). Same rules, no physical object. A row of tokens at the bottom, one query token highlighted, weight-scaled lines fanning to every other token. Caption sits below the fan — clear of every stroke — not inside it.
\`\`\`svg
...
const m = `## Data visualization — the design layer
...
:root {
  ...
  --chart-categorical-2: var(--aqua-350);
  --chart-categorical-3: var(--yellow-250);
  --chart-categorical-4: var(--green-500);
  --chart-categorical-5: var(--violet-600);
  --chart-categorical-6: var(--red-400);
  --chart-categorical-7: var(--magenta-300);
  --chart-categorical-8: var(--orange-350);
...
[data-mode="dark"] {
  ...
  --chart-categorical-2: var(--aqua-400);
  --chart-categorical-3: var(--yellow-350);
  --chart-categorical-4: var(--green-500);
  --chart-categorical-5: var(--violet-350);
  --chart-categorical-6: var(--red-350);
  --chart-categorical-7: var(--magenta-400);
  --chart-categorical-8: var(--orange-400);
...
(() => {
  ...
  var scrubUrlAndHandlerAttrs = (rootEl) => {
    for (const el of Array.from(rootEl.querySelectorAll("*"))) {
      if (el.localName.includes(":")) el.remove();
    }
    const stack = [rootEl];
    while (stack.length) {
      const n = stack.pop();
    ...
    for (const el of [rootEl, ...rootEl.querySelectorAll("*")]) {
      ...
      for (const attr of Array.from(el.attributes)) {
        ...
        AUTO_FETCH_ELEMENTS.has(local) && isRefAttr(name) && !isFragmentOrDataImage(attr.value) || URL_ATTRS.has(name) && !isSafeUrl(attr.value)) {
          ...
          if (isDangerousStyleValue(attr.value)) el.removeAttribute(attr.name);
        } else if (PAINT_ATTRS.has(name) && !isSafePaintValue(attr.value)) {
          if (name === "cursor") el.removeAttribute(attr.name);
          else el.setAttribute(attr.name, "none");
        }
      }
    }
  };
  var hasUnrasterizableRef = (root) => {
    ...
    for (const el of Array.from(root.querySelectorAll(carriers))) {
      for (const name of refAttrs) {
        ...
        if (ref !== null && !isFragmentOrDataImage(ref)) return true;
      }
    }
    return false;
  };
    ...
    try {
      ...
      const freezeSvgForDownload = () => {
        ...
        if (missing.size > 0 && shellDefs) {
          ...
          if (copied) return new XMLSerializer().serializeToString(clone);
        }
        return serialized;
      };
      ...
      const foreignObjectRendersInImage = async () => {
        if (foInImageSupport !== null) return foInImageSupport;
        try {
      ...
      const exportSvgAsPng = async () => {
        const live = container.querySelector("svg");
        if (!live) return null;
        if (hasUnrasterizableRef(live)) return null;
      ...
      if (pngIconEl) pngIconEl.innerHTML = downloadIcon;
      ...
      const acquireDownload = () => {
        ...
        downloadGuardTimer = setTimeout(() => {
          if (epoch === downloadEpoch) downloadInFlight = false;
        }, 6e4);
        return epoch;
      };
...
const I = ["diagram", "mockup", "interactive", "data_viz", "art", "chart", "elicitation"];
const L = ["mobile", "desktop", "unknown"];
function D(e) {
  const t = b(e);
  const n = C(e);
  const a = k(e);
...
const P = [{
  ...
  description: "Renders SVG or HTML content progressively as it streams for the show_widget tool",
...
function _(e) {
      ...
      contents: [{
        ...
        _meta: {
          ui: {
            ...
            csp: {
              connectDomains: ["https://esm.sh", "https://cdnjs.cloudflare.com", "https://cdn.jsdelivr.net", "https://unpkg.com"],
              resourceDomains: ["https://esm.sh", "https://cdnjs.cloudflare.com", "https://cdn.jsdelivr.net", "https://unpkg.com", "https://fonts.googleapis.com", "https://fonts.gstatic.com"]
            }
          }
        }
      }]
  ...
  throw new Error(`Unknown resource: ${e}`);
}
function F() {
  return P;
}
...
function z() {
  return {
    ...
    handleToolCall: async (e, t) => R(e, t),
    handleReadResource: e => _(e),
    handleListResources: () => F(),
    isEnabled: e => (p.isFeatureEnabled("3444158716") || false) && (e.sessionType === "cowork" || e.sessionType === "ccd" && p.isFeatureEnabled("3516166472"))
  };
}
exports.getImagineServerDef = z;
//# sourceMappingURL=index.chunk-DZXOKsI2.js.map