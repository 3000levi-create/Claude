...
const MAX_LINE_LEN = 16777216;
const MAX_TRANSCRIPT_BYTES = 1073741824;
...
async function mapBatched(items, batchSize, fn) {
  ...
  for (let i = 0; i < items.length; i += batchSize) {
    out.push(...(await Promise.all(items.slice(i, i + batchSize).map(x => fn(x)))));
  }
  return out;
}
async function getAllSessionFiles(claudeConfigDir) {
  ...
  const projectResults = await mapBatched(projectDirs, STAT_BATCH_SIZE, async projectDir => {
    try {
      ...
      const subagentResults = await mapBatched(sessionDirs, STAT_BATCH_SIZE, async sd => {
        const subDir = node_path.join(projectDir, sd.name, "subagents");
        try {
          const subs = await node_fs.promises.readdir(subDir, {
            withFileTypes: true
          });
          return subs.filter(s => s.isFile() && s.name.endsWith(".jsonl") && s.name.startsWith("agent-")).map(s => node_path.join(subDir, s.name));
        } catch {
          return [];
        }
      });
...
async function foldTranscript(file, isSubagentFile, fromDate, onSkip) {
  ...
  for await (const line of boundedLines(file, MAX_LINE_LEN, onSkip)) {
    ...
    const content = (_a2 = e.message) == null ? undefined : _a2.content;
    if (Array.isArray(content)) {
      for (const block of content) {
        if ((block == null ? undefined : block.type) === "tool_use") {
          toolCallCount++;
        }
      }
    }
    const usage = (_b = e.message) == null ? undefined : _b.usage;
    const model = ((_c = e.message) == null ? undefined : _c.model) ?? "unknown";
    if (!usage || model === SYNTHETIC_MODEL) {
      continue;
    }
    const mu = usageByModel[model] ??= {
      inputTokens: 0,
      outputTokens: 0,
...
async function computeCodeStats(claudeConfigDir) {
  ...
  const fromDate = (cache == null ? undefined : cache.lastComputedDate) ? dayAfter(cache.lastComputedDate) : toDateString(new Date(Date.now() - COLD_SCAN_DAYS * 24 * 60 * 60 * 1000));
  ...
  for (const d of (cache == null ? undefined : cache.dailyActivity) ?? []) {
    if (d.date < fromDate) {
  ...
  for (const d of (cache == null ? undefined : cache.dailyModelTokens) ?? []) {
    if (d.date < fromDate) {
  ...
  for (const [model, u] of Object.entries((cache == null ? undefined : cache.modelUsage) ?? {})) {
    modelUsage[model] = {
  ...
  for (const [h, c] of Object.entries((cache == null ? undefined : cache.hourCounts) ?? {})) {
    hourCounts.set(Number(h), c);
  }
  let totalSessions = (cache == null ? undefined : cache.totalSessions) ?? 0;
  let totalMessages = (cache == null ? undefined : cache.totalMessages) ?? 0;
  let firstSessionDate = (cache == null ? undefined : cache.firstSessionDate) ?? null;
