...
async function scanFile(session, needle, messageTypes, prefilter) {
  ...
  try {
    for await (const line of rl) {
      ...
      const text = extractText((_a2 = parsed.message) == null ? undefined : _a2.content);
      if (!text) {
        continue;
      }
...
async function handleSearch(port, req) {
  ...
  hits.sort((a, b) => b.lastActivityAt - a.lastActivityAt || (inputOrder.get(a.sessionId) ?? 0) - (inputOrder.get(b.sessionId) ?? 0));
  postToParent(port, {
    type: "result",
...
if ((_a = process.parentPort) == null) {
  ...
  _a.once("message", e => {
    var _a2;
    if (e.data.type !== "init" || !((_a2 = e.ports) == null ? undefined : _a2[0])) {
      process.exit(1);
    }
