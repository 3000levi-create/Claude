...
try {
    (function() {
        var e = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
            o = new e.Error().stack;
        o && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[o] = "09a47c7d-c3f4-4956-adf4-11d9ee47d0fb", e._sentryDebugIdIdentifier = "sentry-dbid-09a47c7d-c3f4-4956-adf4-11d9ee47d0fb")
    })()
} catch {}
const ut = require("node:crypto"),
    ...
    Ve = require("node:os"),
    b = require("node:path"),
    De = require("./index.chunk-ZXPKeP3a.js"),
    t = require("./index.chunk-c42vKsva.js"),
    D = require("node:fs");
require("node:timers/promises");
const dt = require("node:child_process"),
    J = require("./index.chunk-BLNdD7Yt.js"),
    X = require("./index.chunk-CvbeGVMj.js"),
    Ye = require("electron"),
    We = "CLAUDE_CODE_OAUTH_TOKEN",
    ht = "CLAUDE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR",
    de = new Set;
process.once("exit", () => {
    for (const e of de) try {
        e.kill("SIGTERM")
    } catch {}
});
const Ue = 20,
...
function ft(e) {
    const o = [];
    let r = "";
    return e.stderrTail = o, e.stderrPartial = void 0, n => {
        if (t.logger.warn(`[HostLoop] cli.js stderr: ${n.trimEnd()}`), e.stderrTail !== o) return;
        const s = (r + n).split(`
`);
        r = s.pop() ?? "", e.stderrPartial = r || void 0;
        for (const i of s) i.length > 0 && o.push(i.slice(0, mt));
        o.length > Ue && o.splice(0, o.length - Ue)
    }
}

function pt(e = {}) {
    if (process.platform !== "win32") return o => yt(o, e)
}
...
function yt(e, {
    ...
    let l;
    const u = y[We];
    u && (l = gt(u), l !== void 0 && (delete m[We], m[ht] = "3"));
    const h = l !== void 0 ? ["pipe", "pipe", "pipe", l] : ["pipe", "pipe", "pipe"];
    ...
    de.add(a);
    const p = () => de.delete(a);
    a.once("exit", p), a.once("error", p), r == null || r(), (v = a.stdout) == null || v.once("data", () => n == null ? void 0 : n());
    const w = o ?? (_ => t.logger.warn(`[HostLoop] cli.js stderr: ${_.trimEnd()}`));
    return (T = a.stderr) == null || T.on("data", _ => w(_.toString())), {
        stdin: a.stdin,
        stdout: a.stdout,
        get killed() {
            return a.killed
...
function gt(e) {
    ...
    try {
        ...
        const n = b.join(o, "t");
        return D.writeFileSync(n, e, {
            encoding: "utf8",
            mode: 384
        }), r = D.openSync(n, "r"), D.rmSync(n, {
            force: !0
        }), D.rmSync(o, {
            recursive: !0,
            force: !0
        ...
        if (t.logger.warn("[HostLoop] Failed to stage OAuth token fd; falling back to env:", n), r !== void 0) try {
            D.closeSync(r)
        } catch {}
        if (o) try {
            D.rmSync(o, {
...
function qe(e) {
    ...
    let s;
    return r && r.includes(He) ? s = r.replaceAll(He, () => o) : (r && t.logger.warn("cowork_memory_guidelines template is missing {{memoryDir}}; falling back to the in-file default"), s = `You have a persistent, file-based memory system at \`${o}\`. ${vt} ${Pt}`), [s, "", ...n].join(`
`)
}
...
function Xe() {
    ...
    return typeof e == "string" && e.length > 0 ? e : At
}
const kt = /https?:\/\/[^\s<>"'`]+/g,
    Lt = /www\.[^\s<>"'`]+/g,
    $t = new RegExp("(?<!\\S)(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\\.)+[a-zA-Z]{2,63}(?:\\/[^\\s<>\"'`]*)?", "g");
...
function G(e) {
    ...
    return o.protocol !== "http:" && o.protocol !== "https:" ? null : (o.hash = "", o.pathname !== "/" && o.pathname.endsWith("/") && (o.pathname = o.pathname.slice(0, -1)), o.href)
}
const It = new Set(`.,;:!?'"`),
    Ft = {
        ")": "(",
...
function ce(e) {
    ...
    for (; o > 0;) {
        ...
        if (n) {
            let s = 0,
                i = 0;
            for (let c = 0; c < o; c++) e[c] === n ? s++ : e[c] === r && i++;
            if (i > s) {
                o--;
                continue
            }
...
const he = "[webFetch]",
    Y = 3e4,
    ze = `Fetch the contents of a web page at a given URL.
Do not add www. to URLs that do not have them.
URLs must include the scheme: https://example.com is valid, example.com is not.`;

function Dt(e, o) {
    const r = e;
    if (!r || !Array.isArray(r.results)) return t.logger.debug(`${he} ingestWebSearchResultForProvenance: no results[] on input (${r==null?"nullish":typeof r})`), 0;
    const n = o.size;
...
async function Ze(e, o, r, n, s) {
    const i = G(e.url);
    if (!i) return r(!0), K(`Invalid URL: ${e.url}`);
    if (t.permissionModeSkipsProvenance(s == null ? void 0 : s()) && o.add(i), !o.has(i)) {
        const a = new URL(i).hostname;
        if (n && await n(a, i)) o.add(i);
        else return r(!0), K(n ? "Web fetch was not allowed." : "URL not in provenance set. web_fetch can only retrieve URLs that appeared in a user message or a prior web_fetch result. Ask the user to include the URL in a message first.")
    }
    const c = await t.getLastActiveOrg();
    if (!c) return r(!0), K("No active organization");
    const y = e.timeout_ms ?? Y,
        d = new AbortController,
        m = setTimeout(() => d.abort(), y);
    let l;
    try {
        ...
        return r(!0), K(`Fetch failed: ${p}`)
    }
    if (!l.ok) {
        const a = await l.text().catch(() => "");
        return clearTimeout(m), l.status === 403 && a.includes("url_not_allowed") && t.logger.error(`${he} client/server provenance mismatch: client accepted ${le(i)} but server rejected. allowed_urls size=${o.size}`), r(!0), K(`HTTP ${l.status}: ${a}`)
    }
    let u;
    try {
        u = await l.json()
    } catch (a) {
        return clearTimeout(m), r(!0), K(`Failed to parse response: ${a}`)
    }
    if (clearTimeout(m), u.destination_url) {
    ...
    t.logger.info(`${he} via claude.ai: ${le(u.url)}` + (u.destination_url ? ` → ${le(u.destination_url)}` : "") + `, ${u.text.length} chars, ${u.links.length} same-site links`), r(!1);
...
function Ut(e) {
    const o = X.tool(t.WORKSPACE_WEB_FETCH, ze, {
        ...
        const n = Date.now(),
            s = i => void t.logCoworkEvent("lam_mcp_tool_call_completed", {
...
function Bt() {
    return jt ?? (jt = Ye.session.fromPartition("workspace-web-fetch"))
}
...
function Ge(e, o, r, n, s, i) {
    const c = new Map([...o].map(([h, a]) => [t.toGuestCompatibleMountName(a), h])),
        y = new Map((r ?? []).map(h => [t.mountPathOf(h), h.display])),
        d = /[/\s;&|()<>,}:="'$`]/,
        m = (h, a) => {
            let p = -1;
            for (;
...
[Note: ${new Intl.ListFormat("en",{type:"conjunction"}).format(l.map(h=>`
    "${h}"
    `))} ${t.MOUNT_FAILED_TAG_HOST_LOOP}.]`
}
...
function Gt(e, o) {
    ...
    const r = e.toLowerCase(),
        n = o.toLowerCase();
    return n.startsWith("*.") ? r.endsWith(n.slice(1)) : r === n
}

function Qe(e, o) {
    if (e.protocol !== "http:" && e.protocol !== "https:") return `URL scheme "${e.protocol}" is not allowed. Use http or https.`;
    if (t.isLocalOrPrivateHost(e.hostname)) return `Host "${e.hostname}" is a local or private address.`;
    if (!o || o.length === 0) return `No network allowlist is configured for this session (${Ne}). The web_fetch tool is disabled.`;
...
async function Kt(e, o) {
    ...
    let n;
    try {
        return await Promise.race([e, new Promise(i => {
            n = setTimeout(() => i(r), o)
        })]) === r ? "booting" : "ready"
    } catch {
        return "failed"
...
async function xt(e, o) {
    let r;
    const n = i => {
            const c = i == null ? void 0 : i.failedMounts;
            c !== void 0 && (r = c)
        },
        s = () => t.runOneShotInVM({
            ...e,
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                throw r != null && r.length && (l.failedMounts = r), l
            }
        }
    }
}
...
function B(e, o, r, n, s) {
    t.logCoworkEvent("lam_mcp_tool_call_completed", {
        ...
        session_id: e.sessionId,
        session_type: e.sessionType,
        ...(s == null ? void 0 : s.vm_status) && {
            vm_status: s.vm_status
        },
        ...(s == null ? void 0 : s.target_folder_kind) && {
            target_folder_kind: s.target_folder_kind
        },
        ...(s == null ? void 0 : s.vm_was_ready) !== void 0 && {
...
function Vt(e) {
    const o = t.getParsedFeatureValueForKey("1978029737", "workspaceBashWaitLonger", !1, t.booleanType()),
        r = t.getParsedFeatureValueForKey("1978029737", "bashHostOnlyIntercept", !0, t.booleanType()),
        n = X.tool(t.WORKSPACE_BASH, "Run a shell command in the session's isolated Linux workspace. Your connected folders are mounted under /sessions/<session>/mnt/ — the Shell access section of your system prompt lists the exact path for each folder. Each bash call is independent (no cwd/env carryover). Use absolute paths. The workspace boots in the background and may not be ready on the first call; if so, you'll see 'Workspace still starting' — wait a few seconds and retry.", {
            ...
            const d = Date.now(),
                m = e.ensureVmReady(),
                l = e.computeBashMounts(),
                u = (W = e.getResolvedFolders) == null ? void 0 : W.call(e),
                h = `/sessions/${e.vmProcessName}/mnt/`,
                a = new Map;
            for (const [f, P] of l.nameByFolder ?? []) a.set(f, h + P);
            const p = (U = t.findContainingResolvedFolder(c.command, u, a)) == null ? void 0 : U.kind,
                w = r ? t.findBashUnreachableTarget(c.command, u, a, [`/sessions/${e.vmProcessName}`, "/tmp"]) : void 0;
            if (w) {
                const f = t.folderKindHintsOf(w);
                B(e, t.WORKSPACE_BASH, d, !0, {
                    target_folder_kind: p
                });
                const P = (f == null ? void 0 : f.tag) ?? "host-only folder",
                    A = f != null && f.copyHint ? ` ${f.copyHint}` : "";
                return R(`That command targets ${w.display}, which bash cannot reach from the sandbox (${P}).${A}`)
            }
            const v = !Be.has(e.sessionId),
                T = o && v ? Nt : Ht,
                _ = t.getLastVMStartupError(),
                F = ((H = t.getSupportedFeaturesSync().yukonSilver) == null ? void 0 : H.status) !== "supported",
                O = F || _ ? "failed" : await Kt(m, T);
            o && v && O !== "failed" && Be.add(e.sessionId);
            const L = y == null ? void 0 : y.signal;
            if (L != null && L.aborted) return B(e, t.WORKSPACE_BASH, d, !0, {
                vm_status: O
            }), R("Cancelled.");
            if (t.logger.info(`${V} bash: vmStatus=${O} after ${Date.now()-d}ms wait, cmdLen=${c.command.length}, vmCwd=${l.vmCwd}, mounts=${Object.keys(l.mounts).join(",")}`), O === "booting") return B(e, t.WORKSPACE_BASH, d, !0, {
                vm_was_ready: !1,
                vm_status: "booting",
                target_folder_kind: p
            }), R("Workspace still starting. The isolated Linux environment is booting in the background (usually 10–30 seconds). Try again shortly.");
            if (O === "failed") {
                B(e, t.WORKSPACE_BASH, d, !0, {
                    vm_was_ready: !1,
                    vm_status: "failed",
            ...
            try {
                ...
                t.logger.info(`${V} bash done: exit=${f.exitCode}, duration=${Date.now()-d}ms, outputBytes=${f.output.length}`), B(e, t.WORKSPACE_BASH, d, f.exitCode !== 0, {
                    vm_was_ready: !0,
                    vm_status: O,
                    target_folder_kind: p
                });
                let P = "";
                (N = f.failedMounts) != null && N.length && ((E = e.onBashFailedMounts) == null || E.call(e, f.failedMounts, l.nameByFolder ?? new Map, l.mountSetGen), P = Ge(f.failedMounts, l.nameByFolder ?? new Map, u, c.command, l.vmCwd, h));
                const A = f.output.length > 0 ? f.output : "(no output)";
                return f.exitCode === 0 ? Ke(A + P) : R(`Exit code ${f.exitCode}
${A}${P}`)
            } catch (f) {
                const P = f instanceof Error ? f.message : String(f),
                    A = f == null ? void 0 : f.failedMounts;
                let z = "";
                return A != null && A.length && ((C = e.onBashFailedMounts) == null || C.call(e, A, l.nameByFolder ?? new Map, l.mountSetGen), z = Ge(A, l.nameByFolder ?? new Map, u, c.command, l.vmCwd, h)), t.logger.warn(`${V} bash failed:`, f), B(e, t.WORKSPACE_BASH, d, !0, {
                    vm_was_ready: !0,
                    vm_status: O,
                    target_folder_kind: p
                }), R(P + z)
        ...
        s = t.getParsedFeatureValueForKey("1978029737", "coworkWebFetchViaApi", !1, t.booleanType()),
        i = X.tool(t.WORKSPACE_WEB_FETCH, ze, {
            url: t.stringType().url().describe("The URL to fetch."),
            timeout_ms: t.numberType().int().positive().max(Y).optional().describe(`Timeout in milliseconds. Default ${Y}.`)
        }, async c => {
            var u, h, a;
            const y = Date.now(),
                d = p => B(e, t.WORKSPACE_WEB_FETCH, y, p);
            if (s && (e.getWebFetchAllowedUrls().size > 0 || e.requestWebFetchApproval || t.permissionModeSkipsProvenance((u = e.getSdkPermissionMode) == null ? void 0 : u.call(e)))) return Ze(c, e.getWebFetchAllowedUrls(), d, e.requestWebFetchApproval, e.getSdkPermissionMode);
            let m;
            try {
                m = new URL(c.url)
            } catch {
                return d(!0), R(`Invalid URL: ${c.url}`)
            }
            const l = c.timeout_ms ?? Y;
            for (let p = 0; p <= je; p++) {
                const w = Qe(m, e.allowedDomains);
                if (w) return d(!0), R(p === 0 ? w : `Redirect to ${m.href} blocked: ${w}`);
                const v = new AbortController,
                    T = setTimeout(() => v.abort(), l);
                let _;
                try {
                    ...
                    return d(!0), R(`Fetch failed: ${C}`)
                }
                if (_.status >= 300 && _.status < 400) {
                    clearTimeout(T), (h = _.body) == null || h.cancel();
                    const E = _.headers.get("location");
                    if (!E) return d(!0), R(`Redirect ${_.status} from ${m.href} had no Location header.`);
                    try {
                        m = new URL(E, m)
                    } catch {
                        return d(!0), R(`Redirect ${_.status} from ${m.href} had invalid Location: ${E}`)
                    }
                    continue
                }
                const F = (a = _.body) == null ? void 0 : a.getReader(),
                    O = [];
                let L = 0,
                    W = !1;
                if (F) try {
                    for (;;) {
                        ...
                        if (E) break;
                        if (L + C.length > te) {
                            O.push(C.subarray(0, te - L)), L = te, W = !0, F.cancel();
                            break
                        }
                        L += C.length, O.push(C)
                    }
                    ...
                    clearTimeout(T);
                    const C = E instanceof Error ? E.message : String(E);
                    return d(!0), R(`Body read failed after ${L} bytes (${Date.now()-y}ms): ${C}`)
                }
                clearTimeout(T);
...
(Truncated at ${te} bytes.)` : "");
                return d(!_.ok), _.ok ? Ke(`${N}

${H}`) : R(`${N}

${H}`)
            }
            return d(!0), R(`Gave up after ${je} redirects starting from ${c.url}.`)
        });
    return X.createSdkMcpServer({
        name: t.WORKSPACE_MCP_SERVER,
        version: "1.0.0",
...
async function qt(e) {
    if (!Yt.includes(e.toolName)) return;
    const {
        resolved: o,
        inputPath: r
    } = e;
    if (e.isChat && await t.isPathContainedInFolders(o, e.chatWritableRoots)) return;
    ...
    if (await t.isPathContainedInFolders(o, e.uploadsReadOnlyRoots)) return {
        decision: "block",
    ...
    if (await t.isPathContainedInFolders(o, e.spooledProjectsReadOnlyRoots)) return {
        decision: "block",
    ...
    if (!e.isChat && await t.isPathContainedInFolders(o, e.readOnlyPluginPaths)) return {
        decision: "block",
...
async function fe(e) {
    ...
    const o = me,
        r = ut.createHash("sha256").update(e).digest("hex").slice(0, 16),
        n = b.join(o, r);
    return Xt.runExclusive(async () => {
        ...
        try {
            if (await x.readlink(n) === e) return n;
            await x.rm(n, {
                force: !0
            })
        } catch {}
        try {
            ...
            if (s.code === "EEXIST") try {
                if (await x.readlink(n) === e) return n
            } catch {}
            return t.logger.warn(`[HostLoop] Could not stage plugin symlink, falling back to raw path (hooks with unquoted \${CLAUDE_PLUGIN_ROOT} will break): ${n} -> ${e}: ${String(s)}`), e
        }
        return t.logger.info(`[HostLoop] Staged plugin: ${n} -> ${e}`), n
    })
}
async function Je(e) {
    ...
    for (const n of ["known_marketplaces.json", "installed_plugins.json"]) {
        ...
        try {
            const i = await x.readFile(s, "utf-8"),
                c = i.replaceAll(e, () => o);
            c !== i && (await t.writeFileAtomic(s, c), t.logger.info(`[HostLoop] Rewrote ${n} paths through staged config dir`))
        } catch (i) {
            i.code !== "ENOENT" && t.logger.warn(`[HostLoop] Failed to rewrite ${s} through staged config dir:`, i)
        }
    }
    return o
}
...
function xe(e, o) {
    if (t.HOST_LOOP_PATH_GATED_BUILTIN_TOOLS.includes(e))
        for (const r of pe) {
            const n = o[r];
            if (typeof n == "string" && (n === "/sessions" || n.startsWith("/sessions/"))) return t.logger.info(`[canUseTool:HostLoop] ${e} → deny (VM path on host: ${n})`), {
                behavior: "deny",
...
function Qt(e, o, r, n) {
    if (!t.HOST_LOOP_PATH_GATED_BUILTIN_TOOLS.includes(e)) return;
    const s = pe.map(i => o[i]).find(i => typeof i == "string");
    if (s !== void 0) return t.logger.info(`[canUseTool:HostLoop] ${e} → deny (${s}; reason: ${r??"none"})`), r === Zt ? {
        behavior: "deny",
...
const oe = e => `Read(${t.folderToRulePattern(b.join(e,"projects")).replace(/\/\*\*$/,"/**/tool-results/**")})`,
    Jt = ["Write", "Edit", "MultiEdit"];

function eo(e, o, r, n, s, i, c, y = s) {
    const d = c ? t.folderToRulePattern(c) : null;
    return [...(e == null ? void 0 : e.filter(m => !t.HOST_LOOP_EXCLUDED_BUILTIN_TOOLS.includes(m) && !t.HOST_LOOP_PATH_GATED_BUILTIN_TOOLS.includes(m))) ?? [], ...t.WORKSPACE_ALLOWED_TOOLS, ...t.buildFolderPermissionRules([r, ...o]), `Read(${t.folderToRulePattern(n)})`, oe(s), ...y !== s ? [oe(y)] : [], ...t.buildFolderReadRules(i ?? []), ...d ? [`Edit(${d})`, `Write(${d})`, `Read(${d})`] : []]
}
async function et(e, o) {
    var Ce, Pe, Ae, ke, Le, $e, Me;
    const {
        sessionId: r,
        sessionType: n,
        vmProcessName: s,
    ...
    if (!m) throw new Error("Host Claude Code binary not available. Check that the download completed.");
    await t.mkdirPrivate(c), await t.mkdirPrivate(h), await t.mkdirPrivate(b.join(l, "projects"));
    let $ = a;
    if ($) try {
        await t.mkdirPrivate($)
    } catch (g) {
        t.logger.warn(`[HostLoop] Failed to create auto-memory dir ${$} — degrading to no-memory:`, g), $ = null
    }
    const ye = t.getUntrustedLaunchOptions({
        cmd: m,
        args: []
    });
    e.pathToClaudeCodeExecutable = ye.cmd, e.executableArgs = ye.args, e.cwd = c;
    const ge = pt({
        onStderr: f,
        onSpawnConfirmed: P,
        onFirstStdout: A
    });
    ge ? e.spawnClaudeCodeProcess = ge : delete e.spawnClaudeCodeProcess, Array.isArray(e.tools) && (e.tools = zt(e.tools));
    const Z = T(),
        j = await Je(l);
    e.allowedTools = eo(e.allowedTools, Z, u, h, l, re, $, j);
    const _e = (Ce = e.env) == null ? void 0 : Ce.CLAUDE_CODE_GIT_BASH_PATH,
        [we, ae] = await Promise.all([t.allPaths().catch(() => {}), t.getSystemCABundlePath()]);
    e.env = {
        ...e.env,
        ...
        ...$ ? (() => {
            const g = Xe(),
                S = t.isFeatureEnabled("1696890383");
            return {
    ...
    }, e.env = t.mergeNetworkEnv(await t.resolveSystemProxyEnv(i), e.env), e.env = {
        ...e.env,
        ...t.grpcProxyDowngradeEntries(e.env)
    }, e.env.OTEL_EXPORTER_OTLP_PROTOCOL === "grpc" && await De.findSettingsProxySource(c) !== null && (e.env = {
        ...e.env,
        OTEL_EXPORTER_OTLP_PROTOCOL: "http/protobuf"
    });
    const ve = t.grpcTlsRootsForSpawn({
        effectiveProtocol: (Pe = e.env) == null ? void 0 : Pe.OTEL_EXPORTER_OTLP_PROTOCOL,
        existingSpawnValue: (Ae = e.env) == null ? void 0 : Ae.GRPC_DEFAULT_SSL_ROOTS_FILE_PATH,
    ...
    }), Z.length > 0 && (e.additionalDirectories = [...e.additionalDirectories ?? [], ...Z]), e.disallowedTools = [...e.disallowedTools ?? [], ...t.HOST_LOOP_EXCLUDED_BUILTIN_TOOLS], e.toolAliases = {
    ...
    const ie = n === t.SESSION_TYPE_CHAT,
        be = [c, u],
        Ee = [h],
        Te = [b.join(l, "projects"), ...j !== l ? [b.join(j, "projects")] : []],
        nt = [...Ee, ...Te],
        st = ie ? [...be, ...nt] : [c, u, h, b.join(l, "projects"), ...j !== l ? [b.join(j, "projects")] : [], ...$ ? [$] : [], ...se ? [se] : [], ...re ?? [], ...e.additionalDirectories ?? []];
    e.hooks = {
        ...e.hooks,
        PreToolUse: [...((ke = e.hooks) == null ? void 0 : ke.PreToolUse) ?? [], {
            matcher: [...t.HOST_LOOP_PATH_GATED_BUILTIN_TOOLS, "MultiEdit"].join("|"),
            hooks: [async g => {
                if (g.hook_event_name !== "PreToolUse") return {};
                const S = g.tool_input,
                    k = xe(g.tool_name, S);
                if ((k == null ? void 0 : k.behavior) === "deny") return {
                    decision: "block",
                    reason: k.message
                };
                ...
                const q = t.expandTildePath(I.trim()),
                    Ie = b.isAbsolute(q) ? q : b.resolve(c, q);
                let Q;
                try {
                    Q = await t.resolveFilePath(Ie, !0)
                } catch (M) {
                    if ((M == null ? void 0 : M.code) !== "ENOENT") return t.logger.info(`[canUseTool:HostLoop] ${g.tool_name} → block (PreToolUse resolve failed: ${String(M)})`), {
                        decision: "block",
                ...
                const lt = [...st, ...T(), ...ie || ct ? [] : (ne == null ? void 0 : ne()) ?? []];
                return await t.isPathContainedInFolders(Q, lt) ? {} : (t.logger.info(`[canUseTool:HostLoop] ${g.tool_name} → block (PreToolUse: ${I})`), {
                    decision: "block",
    ...
    Se && (e.canUseTool = async (g, S, k) => xe(g, S) ?? Qt(g, S, k.decisionReason, n) ?? Se(g, S, k)), e.debugFile && (e.debugFile = b.join(u, "sdk-debug.txt")), e.stderr = f ?? (g => {
        t.logger.warn(`[HostLoop] cli.js stderr: ${g.trimEnd()}`)
    });
    let Re = !1;
    const at = () => {
            const g = d();
            return Re || (Re = !0, g.catch(S => {
    ...
    return C && t.logCoworkEvent("lam_host_loop_session_started", {
        session_id: r,
        ...
        allowedToolCount: (Le = e.allowedTools) == null ? void 0 : Le.length,
        plugins: e.plugins,
        debugFile: e.debugFile,
        additionalDirectories: e.additionalDirectories,
        envKeys: e.env ? Object.keys(e.env).sort() : [],
        CLAUDE_CONFIG_DIR: ($e = e.env) == null ? void 0 : $e.CLAUDE_CONFIG_DIR,
        model: e.model,
        effort: e.effort,
        maxThinkingTokens: e.maxThinkingTokens,
        CLAUDE_CODE_AUTO_COMPACT_WINDOW: (Me = e.env) == null ? void 0 : Me.CLAUDE_CODE_AUTO_COMPACT_WINDOW,
        autoMemoryHostDir: a
    }), {
        ...
        onFolderAddedForBash: g => {
            ...
            return t.logger.info(`[HostLoop] Folder added: ${g} → /mnt/${k} — next bash call will mount it`), k ?? ""
        },
        stagedConfigDir: j
    }
}
...
function to(e, o, r, n, s, i, c, y, d, m, l) {
    ...
    let w;
    if (e.length === 0) u[a] = p, h = a, w = new Map;
    else {
    ...
    u[t.toGuestCompatibleMountName("uploads")] = {
        ...
        mode: "ro"
    }, y && (u[t.toGuestCompatibleMountName(".claude/skills")] = {
    ...
    for (const {
        ...
        of d ?? []) u[t.toGuestCompatibleMountName(v)] = {
        path: t.guestCompatibleRootPath(T),
        mode: "ro"
    };
    for (const v of l ?? []) v.hostPath && (u[t.toGuestCompatibleMountName(`.projects/${v.uuid}`)] = {
        path: t.guestCompatibleRootPath(v.hostPath),
        mode: "ro"
    });
    return m && (u[t.toGuestCompatibleMountName(t.AUTO_MEMORY_MOUNT_NAME)] = {
