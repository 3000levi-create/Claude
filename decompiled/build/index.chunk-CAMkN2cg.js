...
try {
    (function() {
        var e = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
            n = new e.Error().stack;
        n && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[n] = "20472d88-bb19-43db-be7b-8ea7c600722f", e._sentryDebugIdIdentifier = "sentry-dbid-20472d88-bb19-43db-be7b-8ea7c600722f")
    })()
} catch {}
...
const l = require("node:fs"),
    ...
    u = require("./index.chunk-mJCbQ1X-.js"),
    h = 360 * 60 * 1e3;
let f = null,
    g = !1;
async function x(e = !1) {
    try {
        ...
        if (!(n().isDxtAutoUpdatesEnabled ?? !0) || g) return;
        const {
            getInstalledExtensions: s
        } = await Promise.resolve().then(() => require("./index.chunk-c42vKsva.js")).then(o => o.index), a = (await s({
            forceReload: e
        ...
        g = !0, t.logger.info(`Checking for updates for ${a.length} extensions`);
        try {
            for (const {
                    id: o,
                    manifest: p
        ...
        t.logger.error("Failed to check for extension updates: %o", n), g = !1
    }
}
async function w(e, n) {
    try {
        ...
        } else t.logger.info(`Found update for extension ${e}: ${n.version} -> ${s}`);
        await u.cacheExtensionUpdate(e, s, i), t.logger.info(`Successfully cached update for extension ${e}@${s}${i?" (internal)":""}`)
    } catch (r) {
...
function v() {
    f || (t.logger.info(`Starting periodic extension update checks (interval: ${h}ms)`), f = setInterval(async () => {
        await x(!0)
    }, h), y.app.on("before-quit", () => {
        f && clearInterval(f)
    }), x())
}
let b = !1;
    ...
    if (!b) {
        ...
        try {
            ...
            for (const r of e) {
                ...
                if (!l.existsSync(s)) {
                    t.logger.info(`Skipping cached update for ${r.extensionId}: extension no longer installed`), await u.removeCachedUpdate(r.extensionId);
                    continue
                }
                if (r.isInternalDxt) {
                    const {
            ...
            await c.stat(n).catch(() => null) && (await c.rm(n, {
                recursive: !0,
                force: !0
            }), t.logger.info("Cleaned up extensions update cache directory")), t.logger.info("Successfully applied all cached extension updates")
        } catch (e) {
...
async function E(e) {
    const {
        extensionId: n
    } = e, r = t.getExtensionPath(n), s = t.getCachedExtensionPath(n);
    if (!l.existsSync(s)) {
    ...
    try {
        ...
        l.existsSync(r) && (await c.rename(r, i), t.logger.debug(`Backed up current extension to: ${i}`));
        try {
            await c.rename(s, r), t.logger.info(`Successfully applied update for extension: ${n}`), l.existsSync(i) && (await c.rm(i, {
                recursive: !0,
                force: !0
            }), t.logger.debug(`Cleaned up backup for extension: ${n}`))
        } catch (a) {
            if (t.logger.error(`Failed to apply update for ${n}, rolling back: %o`, a), l.existsSync(i)) try {
                l.existsSync(r) && await c.rm(r, {
                    recursive: !0,
                    force: !0
                }), await c.rename(i, r), t.logger.info(`Successfully rolled back extension: ${n}`)
            } catch (o) {
                t.logger.error(`Failed to rollback extension ${n}: %o`, o)
            }
            throw a
        }
        if (e.signatureInfo && e.hash) try {
            await t.addExtensionMetadata({
                id: n,
                version: e.version,
                hash: e.hash,
                installedAt: new Date().toISOString(),
                certificateFingerprint: e.signatureInfo.status === "signed" ? e.signatureInfo.fingerprint : void 0,
                manifest: e.manifest,
