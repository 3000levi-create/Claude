...
try {
    (function() {
        var e = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
            t = new e.Error().stack;
        t && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[t] = "892e1d7f-c1a9-4c23-8ebc-d60069d59375", e._sentryDebugIdIdentifier = "sentry-dbid-892e1d7f-c1a9-4c23-8ebc-d60069d59375")
    })()
} catch {}
...
function U(e) {
    ...
    let n = "",
        r = t + 1;
    for (; r < e.length && e[r] !== '"';) e[r] === "\\" && r + 1 < e.length ? (n += e[r + 1], r += 2) : (n += e[r], r++);
    return n
}

function x(e) {
    return e ? e.startsWith('"') ? B(e) : E(e).trim() : ""
}
...
function E(e) {
    for (let t = 1; t < e.length; t++)
        if ((e[t] === "#" || e[t] === ";") && /\s/.test(e[t - 1])) return e.slice(0, t);
    return e
}
async function G(e) {
    try {
...
async function v(e) {
    ...
    return await c.assertNoUncSymlinkHop(n), n
}
async function g(e) {
    ...
    try {
        ...
        if (n.isFile()) {
            ...
            if (!i) return null;
            const o = i[1].trim(),
                s = a.isAbsolute(o) ? o : a.resolve(e, o);
            if (await c.assertNoUncSymlinkHop(s), !(await d.promises.stat(s)).isDirectory()) return null;
            const u = await v(s) ?? s;
            return {
                gitDir: s,
                commonDir: u
...
async function q(e) {
    ...
    const n = a.join(e, ".git"),
        r = a.resolve(t.commonDir);
    return r !== a.resolve(n) && r !== a.resolve(t.gitDir) && a.basename(r) === ".git" ? a.dirname(r) : e
}
async function T(e) {
    try {
        const n = (await d.promises.readFile(a.join(e, "HEAD"), "utf-8")).trim(),
            r = n.match(/^ref:\s*refs\/heads\/(.+)$/);
        return r && r[1] ? r[1] : /^[0-9a-f]{40,64}$/i.test(n) ? "HEAD" : null
    } catch {
        return null
    }
}
async function H(e, t) {
    ...
    return (r == null ? void 0 : r.url) ?? null
}
async function J(e, t, n) {
    try {
        if ((await d.promises.lstat(a.join(e, "refs", "remotes", t, n))).isFile()) return !0
    } catch {}
    try {
        const r = a.join(e, "packed-refs");
        if (!(await d.promises.lstat(r)).isFile()) return !1;
...
async function j(e, t) {
    try {
        ...
        if ((await d.promises.lstat(n)).isFile()) {
            ...
            if (i != null && i[1]) return i[1]
        }
    } catch {}
    for (const n of ["main", "master"])
        if (await J(e, t, n)) return n;
    return null
}
async function K(e) {
    try {
...
async function Q(e) {
    const t = await g(e);
    return t ? j(t.commonDir, "origin") : null
}
async function V(e) {
    const t = await g(e);
    return t ? T(t.gitDir) : null
}
...
async function Z(e) {
    const t = await g(e);
    if (!t) return [];
    const [n, r] = await Promise.all([S(a.join(t.commonDir, "refs", "heads")), X(t.commonDir)]), i = new Set(n);
...
function A(e) {
    ...
    return t.length > tt ? void 0 : t
}
async function F() {
    var o;
    const e = c.loadUserEnvVars().CLAUDE_CODE_TMPDIR ?? process.env.CLAUDE_CODE_TMPDIR ?? (process.platform === "darwin" ? "/tmp" : C.tmpdir()),
        t = (o = process.getuid) == null ? void 0 : o.call(process),
        n = process.platform === "win32" ? "claude" : `claude-${t??0}`,
        r = a.join(e, n);
    ...
    try {
        ...
        const l = s.code;
        return l === "ENOENT" || l === "ENOTDIR" ? {
            dir: r,
            verified: !1
        } : void 0
    }
    if (!(!i.isDirectory() || t !== void 0 && i.uid !== t)) try {
        return {
...
function _() {
    return process.env.CLAUDE_CONFIG_DIR ? a.join(c.getClaudeConfigDir(), ".claude.json") : a.join(C.homedir(), ".claude.json")
}
async function D(e) {
    if (!a.isAbsolute(e)) return e;
    try {
        const t = await k(e);
        if (t) return c.canonicalizeWslPath(await f.realpath(t))
    } catch {}
...
function h(e, t) {
    if (!e) return;
    const n = e[t];
    if (n || !c.isWslUncPath(t)) return n;
    for (const [r, i] of Object.entries(e))
        if (c.isWslUncPath(r) && c.canonicalizeWslPath(r) === t) return i
}
const p = new Map;
async function O(e, t) {
    ...
    const i = p.get(e);
    if (i && i.mtimeMs === n && i.size === r) return {
        ...t,
        ...i.parsed
    };
    try {
        ...
        const s = (o == null ? void 0 : o.code) ?? (o == null ? void 0 : o.name) ?? "unknown";
        return c.logger.error(`Failed to parse config file ${e} (${s})`), {
            ...t
        }
    }
}
...
function et(e) {
    ...
    const t = e;
    return (t.type === void 0 || t.type === "stdio") && typeof t.command == "string"
}
async function I(e) {
    const t = await b(),
        n = {},
        r = i => {
            if (!(typeof i != "object" || i === null))
                for (const [o, s] of Object.entries(i)) et(s) && (n[o] = {
                    command: s.command,
                    args: s.args,
                    env: s.env
                })
    ...
    if (r(t.mcpServers), e) {
        const i = await D(e),
            o = h(t.projects, i) ?? L;
        r(o.mcpServers);
        for (const s of o.disabledMcpServers ?? []) delete n[s]
    }
    return n
}
...
async function st(e) {
    ...
    for (;;) {
        try {
            ...
            if (s.code !== "EEXIST") return c.logger.warn(`Cannot create ${t}: ${s}; writing without it`), async () => {}
        }
        const o = await f.stat(t).catch(() => null);
        if (o && Date.now() - o.mtimeMs > it && await n(), Date.now() > r) return c.logger.warn(`Timed out waiting for ${t}; writing without it`), async () => {};
...
async function at(e, t) {
    ...
    try {
        ...
        const o = await O(n, y),
            ...
            l = Object.fromEntries(Object.entries(s).filter(([u, m]) => JSON.stringify(m) !== JSON.stringify(y[u])));
        try {
            const u = `${n}.backup`;
            await f.copyFile(n, u)
        } catch (u) {
            u.code !== "ENOENT" && c.logger.warn(`Failed to backup config: ${u}`)
        }
        await c.writeJsonAtomic(r, l), p.delete(n)
    } catch (o) {
        throw c.logger.error(`Failed to save config: ${o}`), o
    } finally {
        await i()
    }
}
async function P(e, t = !1) {
    var s;
    const n = await b();
    if (!a.isAbsolute(e)) return ((s = h(n.projects, e)) == null ? void 0 : s.hasTrustDialogAccepted) === !0;
    ...
    const i = await D(e),
        o = h(n.projects, i);
    if (o != null && o.hasTrustDialogAccepted) return !0;
    for (; r !== null;) {
        const l = h(n.projects, r);
...
async function ct() {
    const e = await b();
    if (!e.projects) return [];
    const t = Object.entries(e.projects).filter(([, r]) => (r == null ? void 0 : r.hasTrustDialogAccepted) === !0).map(([r]) => r);
    return (await Promise.all(t.map(async r => {
        const i = c.canonicalizeWslPath(r);
        if (c.isUnsafeUnc(i)) return null;
        try {
            return await f.access(i), r
        } catch {
            return null
        }
    }))).filter(r => r !== null)
