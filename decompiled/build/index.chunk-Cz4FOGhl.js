...
try {
    (function() {
        var e = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
            n = new e.Error().stack;
        n && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[n] = "5f545dc5-4895-420b-abb5-520e2650842b", e._sentryDebugIdIdentifier = "sentry-dbid-5f545dc5-4895-420b-abb5-520e2650842b")
    })()
} catch {}
...
const t = require("./index.chunk-c42vKsva.js"),
    G = require("electron"),
    N = ["https://cognitiveservices.azure.com/.default", "offline_access"],
    P = N.join(" "),
    j = "urn:ietf:params:oauth:grant-type:device_code",
    z = 1e4,
    H = 1e4,
    Y = 1e4,
    S = 5 * 6e4,
    ...
    A = 24 * 3600,
    O = 64 * 1024,
    w = (e, n) => G.net.fetch(e, n);

function l(e, n, r, a) {
    const o = typeof e == "number" ? e : typeof e == "string" ? Number(e) : NaN;
    return Number.isFinite(o) ? Math.min(Math.max(Math.trunc(o), n), r) : a
}
...
function F(e) {
    ...
    return n + l(r, R, A, 3600) * 1e3
}

function M(e) {
    const n = typeof e == "number" ? e : Number(e);
    return Number.isFinite(n) && n > 0 ? Date.now() + n * 1e3 : void 0
}
async function p(e) {
    var o;
    const n = (o = e.body) == null ? void 0 : o.getReader();
    if (!n) return JSON.parse(await e.text());
    const r = [];
    let a = 0;
    for (;;) {
        const {
        ...
        if (i) break;
        if (a += c.length, a > O) throw n.cancel(), new Error(`Foundry Entra: response body exceeds ${O} bytes`);
        r.push(c)
    }
...
function x(e, n) {
    ...
    return n ? AbortSignal.any([r, n]) : r
}
...
const q = t.objectType({
    ...
    B = new Set(["authorization_pending", "slow_down"]);
async function J(e, n, r = w) {
    ...
    if (!a.ok) throw new Error(`Foundry Entra device init failed: HTTP ${a.status}`);
    const o = q.safeParse(await p(a));
    if (!o.success) throw new Error(`Foundry Entra device init: ${o.error.issues.map(d=>`${d.path.join(".")}: ${d.message}`).join("; ")}`);
    const i = o.data;
    if ((E = n.cancelled) != null && E.call(n)) throw new Error("Foundry Entra device grant cancelled");
    (m = n.onUserCode) == null || m.call(n, {
        userCode: i.user_code,
        verifyUrl: i.verification_uri
    });
    let c = l(i.interval, V, v, 5) * 1e3;
    const f = Date.now() + Math.min(l(i.expires_in, 30, S / 1e3, 600) * 1e3, S),
        C = _(e.tenantId);
    for (;;) {
        if ((k = n.cancelled) != null && k.call(n)) throw new Error("Foundry Entra device grant cancelled");
        const d = f - Date.now();
        if (d <= 0) break;
        if (await t.sleep(Math.min(c, d)), (g = n.cancelled) != null && g.call(n)) throw new Error("Foundry Entra device grant cancelled");
        let h, T;
        try {
            h = await r(C, {
                method: "POST",
        ...
        if (h.ok) {
            ...
            if (!s.success) throw new Error(`Foundry Entra token response invalid: ${(I=s.error.issues[0])==null?void 0:I.message}`);
            return {
        ...
        const b = $.safeParse(T),
            u = b.success ? b.data.error : void 0;
        if (u && !B.has(u)) throw new Error(`Foundry Entra device grant rejected: ${u}`);
        u === "slow_down" && (c = Math.min(c + 5e3, v * 1e3)), u || t.logger.debug("[custom-3p:entra] token poll non-OAuth response", {
            status: h.status
        })
    }
...
async function X(e, n = w) {
    ...
    if (!r.ok) {
        const c = $.safeParse(a),
            f = c.success ? c.data.error : void 0;
        if (f === "invalid_grant") return {
            kind: "clear"
        };
        throw new Error(`Foundry Entra refresh failed: ${f??`HTTP ${r.status}`}`)
    }
    const o = L.safeParse(a);
    if (!o.success) throw new Error(`Foundry Entra refresh response invalid: ${(i=o.error.issues[0])==null?void 0:i.message}`);
    return {
        kind: "fresh",
        token: {
            ...
            refreshTokenExpiresAt: M(o.data.refresh_token_expires_in) ?? (o.data.refresh_token ? void 0 : e.refreshTokenExpiresAt),
            tenantId: e.tenantId,
            clientId: e.clientId
        }
    }
